...
class OneDBioPINN:
    ...
    def __init__(self, X_measurement_vessel1, 
                ...
                X1_interface, X2_interface,
                T_interface, layers, bif_points):
        ...
        X1_interface = X1_interface/self.L
        X2_interface = X2_interface/self.L
        
        T_residual  = T_residual/self.T
        T_measurement  = T_measurement/self.T
        T_interface = T_interface/self.T
        ...
        self.X1_i = (X1_interface - self.Xmean1)/self.Xstd1
        self.X2_i = (X2_interface - self.Xmean1)/self.Xstd1
        

        self.T_u = (T_measurement - self.Tmean)/self.Tstd
        self.T_f = (T_residual - self.Tmean)/self.Tstd
        self.T_i = (T_interface - self.Tmean)/self.Tstd
        ...
        self.X1_i_tf = tf.placeholder(tf.float32, shape=(None, self.X1_i.shape[1]),name= 'self.X1_i_tf')

        self.X2_i_tf = tf.placeholder(tf.float32, shape=(None, self.X2_i.shape[1]),name= 'self.X2_i_tf')

        self.t_i_tf = tf.placeholder(tf.float32, shape=(None, self.T_i.shape[1]), name='self.t_i_tf')
        ...
        self.A1_i_pred1, self.u1_i_pred1, self.p1_i_pred1  = self.neural_net_vessel1(self.X1_i_tf, self.t_i_tf)

        self.A2_i_pred1, self.u2_i_pred1, self.p2_i_pred1  = self.neural_net_vessel1(self.X2_i_tf, self.t_i_tf)
        ...
        self.loss_i = self.compute_interface_loss_vessel1(self)
...
if __name__ == "__main__":
    ...
    velocity_measurements_vessel1 = velocity_measurements_vessel1[t<=1][:,None]
    
    area_measurements_vessel1 = input_vessel_1["Area"][:,None]
    area_measurements_vessel1 = area_measurements_vessel1[t<=1][:,None]
        
    velocity_test_vessel1 = test_vessel_1["Velocity"][:,None]
    velocity_test_vessel1 = velocity_test_vessel1[t<=1][:,None]

    pressure_test_vessel1 = test_vessel_1["Pressure"][:,None]
    pressure_test_vessel1 = pressure_test_vessel1[t<=1][:,None]
    ...
    X1_interface = 0.0*np.ones((N_i))[:,None]
    X2_interface = 0.1703*np.ones((N_i))[:,None]

    T_interface = lower_bound_t + (upper_bound_t-lower_bound_t)*np.random.random((N_i))[:,None]
    ...
    model = OneDBioPINN(X_measurement_vessel1, 
                        ...
                        X_residual_vessel1,  
                        T_residual, T_measurement, 
                        X1_interface, X2_interface, T_interface,
                        layers, bif_points)

    model.train(nIter = 2000, learning_rate = 1e-3)
    ...
    fig1 = plt.figure(1, facecolor='w', frameon = False)
    fig2 = plt.figure(2, facecolor='w', frameon = False)
